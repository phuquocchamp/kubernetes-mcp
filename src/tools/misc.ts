...
export function registerMiscTools(server: McpServer, deps: Deps): string[] {
  ...
  server.registerTool(
    "explain_resource",
    {
      ...
      inputSchema: {
        resource: z
          .string()
          .min(1)
          .describe("Resource name or field path (e.g. 'pods' or 'pods.spec.containers')"),
        apiVersion: z.string().optional().describe("API version to use (e.g. 'apps/v1')"),
        recursive: z
          .boolean()
          .optional()
          .default(false)
          .describe("Print the fields of fields recursively"),
        context: contextSchema,
        output: z
    ...
    async (args) =>
      runTool("explain_resource", async () => formatExplain(await explain(deps, args))),
  );
  ...
  server.registerTool(
    ...
    async (args) =>
      runTool("list_api_resources", async () =>
        formatApiResources(await listApiResources(deps, args)),
      ),
  );
