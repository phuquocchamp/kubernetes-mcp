...
vi.mock("node:child_process", () => {
  return {
    spawn: vi.fn(() => {
      ...
      setImmediate(() => child.stdout.emit("data", Buffer.from("Forwarding from 127.0.0.1:8080 -> 80\n")));
      return child;
    }),
  };
});
...
describe("portForward", () => {
  test("spawns kubectl with the expected argv and tracks the process", async () => {
    ...
    expect(spawn).toHaveBeenCalledWith("kubectl", ["port-forward", "-n", "prod", "deployment/web", "8080:80"]);
    expect(result).toEqual({ success: true, message: "port-forwarding was successful", id: "deployment-web-8080" });
    expect(deps.client.trackPortForward).toHaveBeenCalledWith(
      expect.objectContaining({ id: "deployment-web-8080", resourceType: "deployment", name: "web", namespace: "prod" }),
    );
  });

  test("omits -n when no namespace is given", async () => {
    const deps = fakeDeps();
    await portForward(deps, { resourceType: "pod", resourceName: "web", localPort: 3000, targetPort: 3000 });
  ...
  test("rejects a flag-like argv before spawning (assertSafeArgv)", async () => {
    const deps = fakeDeps();
    await expect(
      portForward(deps, { resourceType: "--server=evil", resourceName: "web", localPort: 8080, targetPort: 80 }),
    ).rejects.toThrow();
  });
});
...
describe("stopPortForward", () => {
  test("stops and untracks a tracked port-forward", async () => {
    const deps = fakeDeps();
    await portForward(deps, { resourceType: "pod", resourceName: "web", localPort: 8080, targetPort: 80 });
  ...
  test("throws a not_found KubectlError for an unknown id", async () => {
    ...
    await expect(stopPortForward(deps, { id: "missing" })).rejects.toMatchObject({ code: "not_found" });
  });
});
